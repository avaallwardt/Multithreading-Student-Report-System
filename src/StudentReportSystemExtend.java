...
public class StudentReportSystemExtend extends Thread {
    /*
        Your goal is this:
    ...
    public void run(){
        if(isGPA){
            ...
            findStudentsWithNumAbsences();
        }
    }

    public StudentReportSystemExtend(double minGPA, double maxGPA, boolean isFront, ArrayList<Student> theStudentList){
        isGPA = true;
        this.minGPA = minGPA;
        this.maxGPA = maxGPA;
    ...
    public StudentReportSystemExtend(int numAbsences, boolean isFront, ArrayList<Student> theStudentList){
        isGPA = false;
         ...
         * numOfAbsences >= 0 && <= 10
         */
        public void findStudentsWithNumAbsences() {
            if (isFront) {
                for (int i = 0; i < studentList.size() / 2; i++) {
                    System.out.println("Checking front: " + isFront);
